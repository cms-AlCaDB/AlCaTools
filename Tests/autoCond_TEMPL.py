...
autoCond = { 
    ...
    'run1_design'       :   '.oO[RUN1_DESIGN]Oo.',
    # GlobalTag for MC production (pp collisions) with realistic alignment and calibrations for Run1
    'run1_mc'           :   '.oO[RUN1_MC]Oo.',
    # GlobalTag for MC production (Heavy Ions collisions) with realistic alignment and calibrations for Run1
    'run1_mc_hi'        :   '.oO[RUN1_MC_HI]Oo.',
    # GlobalTag for MC production (p-Pb collisions) with realistic alignment and calibrations for Run1
    'run1_mc_pa'        :   '.oO[RUN1_MC_PA]Oo.',
    # GlobalTag for MC production with perfectly aligned and calibrated detector for Run2
    'run2_design'       :   '.oO[RUN2_DESIGN]Oo.',
    # GlobalTag for MC production with pessimistic alignment and calibrations for Run2
    'run2_mc_50ns'      :   '.oO[RUN2_MC_50NS]Oo.',
    #GlobalTag for MC production with optimistic alignment and calibrations for Run2
    'run2_mc'           :   '.oO[RUN2_MC]Oo.',
    # GlobalTag for MC production (Heavy Ions collisions) with optimistic alignment and calibrations for Run2
    'run2_mc_hi'        :   '.oO[RUN2_MC_HI]Oo.',
    # GlobalTag for Run1 data reprocessing
    'run1_data'         :   '.oO[RUN1_DATA]Oo.',
    # GlobalTag for Run2 data reprocessing
    'run2_data'         :   '.oO[RUN2_DATA]Oo.',
    # GlobalTag for Run1 HLT: it points to the online GT
    'run1_hlt'          :   '.oO[RUN1_HLT]Oo.',
    # GlobalTag for Run2 HLT: it points to the online GT
    'run2_hlt'          :   '.oO[RUN2_HLT]Oo.',
    # GlobalTag for MC production with perfectly aligned and calibrated detector for Phase1 2017
    'phase1_2017_design' :  '.oO[PHASE1_2017_DESIGN]Oo.',
