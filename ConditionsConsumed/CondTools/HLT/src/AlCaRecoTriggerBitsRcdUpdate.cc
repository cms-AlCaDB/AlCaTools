...
AlCaRecoTriggerBitsRcdUpdate::AlCaRecoTriggerBitsRcdUpdate(const edm::ParameterSet& cfg)
  : nEventCalls_(0), 
    ...
    alcarecoClone_(cfg.getParameter<std::vector<edm::ParameterSet> >("alcarecoClone"))
{
}
...
void AlCaRecoTriggerBitsRcdUpdate::analyze(const edm::Event& evt, const edm::EventSetup& iSetup)
{
  ...
  // now clone keys 
  this->cloneKeysFromMap(alcarecoClone_,bitsToWrite->m_alcarecoToTrig);
						      ...
						      TriggerMap &triggerMap) const
{
  ...
  for(auto& iKey : keyPairs){
    if(triggerMap.find(iKey.first) != triggerMap.end() ){
      ...
      edm::LogWarning("AlCaRecoTriggerBitsRcdUpdate") << "[AlCaRecoTriggerBitsRcdUpdate::cloneKeysFromMap] "
						      << "Cannot clone key '" << iKey.first << "with " << iKey.second << " since not in "
						      << "list - typo in configuration?\n";
      return false;
    }
  }
