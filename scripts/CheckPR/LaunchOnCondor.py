...
def CreateTheShellFile(argv):
    ...
    if Jobs_RunHere==0:
        ...
        if(not os.path.isabs(Path_Shell)):
...
def AddJobToCmdFile():
    ...
    if subTool=='bsub':
        ...
        if(not os.path.isabs(absoluteShellPath)):
            absoluteShellPath= os.getcwd() + "/"+absoluteShellPath
...
def SendCluster_Create(FarmDirectory, JobName):
    ...
    if(subTool==''):
        if(  not commands.getstatusoutput("which bjobs")[1].startswith("which:")):
            subTool = 'bsub'
        elif( not commands.getstatusoutput("which qsub")[1].startswith("which:")):
            subTool = 'qsub'
        else:
            subTool = 'condor'
    if(Jobs_Queue.find('crab')>=0):
        subTool = 'crab'
